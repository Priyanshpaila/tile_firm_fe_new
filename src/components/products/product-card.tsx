...
type ProductWithWishlistMeta = Product & {
...
export function ProductCard({
  ...
  const productWithMeta = product as ProductWithWishlistMeta;

  const [wishlisted, setWishlisted] = useState(
    Boolean(
      productWithMeta.isWishlisted ?? productWithMeta.inWishlist ?? false,
    ),
  );
  ...
  const chips = useMemo(
    ...
    [product.finishes, product.usages],
  );
  ...
  const handleToggleWishlist = async (
    ...
    if (!isUserRole || wishlistLoading) return;
    ...
    try {
      const res = await api.users.toggleWishlist(product._id);
      setWishlisted(Boolean(res.data.added));
    } catch {
      setWishlisted(previous);
    } finally {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-[26px] border border-black/6 bg-white shadow-[0_14px_36px_rgba(30,20,10,0.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_52px_rgba(30,20,10,0.12)]">
      {isUserRole ? (
        <button
          type="button"
          onClick={handleToggleWishlist}
          aria-label={wishlisted ? "Remove from wishlist" : "Add to wishlist"}
          className="absolute right-3 top-3 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border border-black/8 bg-white/92 text-[var(--text-primary)] shadow-sm backdrop-blur transition hover:scale-[1.03] hover:bg-white"
        >
          {wishlistLoading ? (
      ...
      {onOpenDetails ? (
        ...
        <Link
          href={`/product?slug=${encodeURIComponent(product.slug || product._id)}`}
